...
alignas(CV1K_CACHE_ALIGN) cv1k_u8 cv1k_mame_colrtable[0x20][0x40];
alignas(CV1K_CACHE_ALIGN) cv1k_u8 cv1k_mame_colrtable_rev[0x20][0x40];
alignas(CV1K_CACHE_ALIGN) cv1k_u8 cv1k_mame_colrtable_add[0x20][0x20];
...
void cv1k_mame_build_color_tables(void)
{
    ...
    for (y = 0; y < 0x40; y++) {
        for (x = 0; x < 0x20; x++) {
            cv1k_mame_colrtable[x][y] = clamp5(((cv1k_u32)x * (cv1k_u32)y) / 0x1fUL);
            cv1k_mame_colrtable_rev[x ^ 0x1f][y] = clamp5(((cv1k_u32)x * (cv1k_u32)y) / 0x1fUL);
        }
    }
    for (y = 0; y < 0x20; y++) {
        for (x = 0; x < 0x20; x++) {
            cv1k_mame_colrtable_add[x][y] = clamp5((cv1k_u32)x + (cv1k_u32)y);
        }
    }
    tables_ready = 1;
}
...
cv1k_u8 cv1k_mame_mul5(cv1k_u8 x, cv1k_u8 y)
{
    ...
    return cv1k_mame_colrtable[x & 0x1fU][y & 0x3fU];
}

cv1k_u8 cv1k_mame_mul5_rev(cv1k_u8 x, cv1k_u8 y)
{
    if (!tables_ready) cv1k_mame_build_color_tables();
    return cv1k_mame_colrtable_rev[x & 0x1fU][y & 0x3fU];
}

cv1k_u8 cv1k_mame_add5(cv1k_u8 x, cv1k_u8 y)
{
    if (!tables_ready) cv1k_mame_build_color_tables();
    return cv1k_mame_colrtable_add[x & 0x1fU][y & 0x1fU];
}
