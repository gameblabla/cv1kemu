 ...
 *
 * C23 port of the MAME-derived interpreter wrapper.  The opcode semantics in
 * src/sh_common_ops.inc remain extracted from MAME; this file provides a C
 * dispatch/object context instead of the previous C++ class wrapper.
 *
...
static _Thread_local struct sh34_cpu cv1k_sh34_ctx;

#define st (cv1k_sh34_ctx.st)
#define bus (cv1k_sh34_ctx.bus)
#define cur_opcode (cv1k_sh34_ctx.cur_opcode)
...
static CV1K_ALWAYS_INLINE cv1k_u8 *hot_ptr(uint32_t addr, int is_write, uint32_t need)
{
...
/* SH7709S cache-access timing hook.  In the accurate build the call is
 * gated on the runtime flags inline so the hot RAM/ROM path pays just a
 * predictable, never-taken branch instead of a cross-TU call on every
 * memory access.  In the default (fast) build the whole hook compiles to
 * nothing - see CV1K_CACHE_ACCURATE in cv1k_config.h. */
static CV1K_ALWAYS_INLINE void cache_note(uint32_t a, int is_write, int is_fetch)
{
#if CV1K_CACHE_ACCURATE
    struct cv1k_machine *mm = bus->machine;
    if (CV1K_UNLIKELY(mm->mame_cache_meta || mm->sh7709s_cache_timing))
        cv1k_bus_cache_access(bus, a, is_write, is_fetch);
#else
    (void)a; (void)is_write; (void)is_fetch;
#endif
}
static CV1K_ALWAYS_INLINE uint8_t  read_byte(uint32_t a)
{ cv1k_u8 *p = hot_ptr(a, 0, 1); if (p) { cache_note(a, 0, 0); return *p; } return cv1k_bus_read8(bus, a); }
static CV1K_ALWAYS_INLINE uint16_t read_word(uint32_t a)
{ cv1k_u8 *p = hot_ptr(a, 0, 2); if (p) { cache_note(a, 0, 0); return (uint16_t)(((uint16_t)p[0] << 8) | p[1]); } return cv1k_bus_read16(bus, a); }
static CV1K_ALWAYS_INLINE uint32_t read_long(uint32_t a)
{ cv1k_u8 *p = hot_ptr(a, 0, 4); if (p) { cache_note(a, 0, 0); return (((uint32_t)p[0] << 24) | ((uint32_t)p[1] << 16) | ((uint32_t)p[2] << 8) | p[3]); } return cv1k_bus_read32(bus, a); }
static CV1K_ALWAYS_INLINE void     write_byte(uint32_t a, uint8_t d)
{ cv1k_u8 *p = hot_ptr(a, 1, 1); if (p) { cache_note(a, 1, 0); *p = d; } else cv1k_bus_write8(bus, a, d); }
static CV1K_ALWAYS_INLINE void     write_word(uint32_t a, uint16_t d)
{ cv1k_u8 *p = hot_ptr(a, 1, 2); if (p) { cache_note(a, 1, 0); p[0] = (cv1k_u8)(d >> 8); p[1] = (cv1k_u8)d; } else cv1k_bus_write16(bus, a, d); }
static CV1K_ALWAYS_INLINE void     write_long(uint32_t a, uint32_t d)
{ cv1k_u8 *p = hot_ptr(a, 1, 4); if (p) { cache_note(a, 1, 0); p[0] = (cv1k_u8)(d >> 24); p[1] = (cv1k_u8)(d >> 16); p[2] = (cv1k_u8)(d >> 8); p[3] = (cv1k_u8)d; } else cv1k_bus_write32(bus, a, d); }
static CV1K_ALWAYS_INLINE uint16_t fetch_word(uint32_t a)
{ cv1k_u8 *p = hot_ptr(a, 0, 2); if (p) { cache_note(a, 0, 1); return (uint16_t)(((uint16_t)p[0] << 8) | p[1]); } return cv1k_bus_fetch16(bus, a); }


static void ILLEGAL(void);
static void TRAPA(uint32_t i);
static void LDCSR(uint16_t opcode);
static void LDCMSR(uint16_t opcode);
static void RTE(void);
static void execute_one_0000(uint16_t opcode);
static void execute_one_4000(uint16_t opcode);
static void execute_one_f000(uint16_t opcode_unused);
...
static void TODO(uint16_t opcode_unused) { (void)opcode_unused; }

static void LDTLB(uint16_t opcode_unused)
{
    (void)opcode_unused;
    cv1k_bus_ldtlb(bus); /* MMU disabled on CV1000; keep bus TLB hook */
}

static void MOVCAL(uint16_t opcode)
{
    st->ea = st->r[REG_N];
    write_long(st->ea, st->r[0]);
}

static void CLRS(uint16_t opcode_unused) { (void)opcode_unused; st->sr &= ~SH_S; }
static void SETS(uint16_t opcode_unused) { (void)opcode_unused; st->sr |=  SH_S; }

static void PREFM(uint16_t opcode_unused)
{
    (void)opcode_unused;
...
/*  STC Rm_BANK,Rn  / @-Rn */
static void STCRBANK(uint16_t opcode)
{
    st->r[REG_N] = st->rb[REG_M & 7];
}
static void STCMRBANK(uint16_t opcode)
{
    uint32_t n = REG_N;
    st->r[n] -= 4;
    st->ea = st->r[n];
...
static void LDCRBANK(uint16_t opcode)
{
    st->rb[REG_M & 7] = st->r[REG_N];
}
static void LDCMRBANK(uint16_t opcode)
{
    uint32_t n = REG_N;
    st->ea = st->r[n];
...
/*  SGR / SSR / SPC moves */
static void STCSGR(uint16_t opcode) { st->r[REG_N] = st->sgr; }
static void STCSSR(uint16_t opcode) { st->r[REG_N] = st->ssr; }
static void STCSPC(uint16_t opcode) { st->r[REG_N] = st->spc; }
static void LDCSSR(uint16_t opcode) { st->ssr = st->r[REG_N]; }
static void LDCSPC(uint16_t opcode) { st->spc = st->r[REG_N]; }

static void STCMSGR(uint16_t opcode)
{ uint32_t n = REG_N; st->r[n] -= 4; st->ea = st->r[n]; write_long(st->ea, st->sgr); }
static void STCMSSR(uint16_t opcode)
{ uint32_t n = REG_N; st->r[n] -= 4; st->ea = st->r[n]; write_long(st->ea, st->ssr); }
static void STCMSPC(uint16_t opcode)
{ uint32_t n = REG_N; st->r[n] -= 4; st->ea = st->r[n]; write_long(st->ea, st->spc); }
static void LDCMSSR(uint16_t opcode)
{ st->ea = st->r[REG_N]; st->ssr = read_long(st->ea); st->r[REG_N] += 4; }
static void LDCMSPC(uint16_t opcode)
{ st->ea = st->r[REG_N]; st->spc = read_long(st->ea); st->r[REG_N] += 4; }

/*  SHAD / SHLD (SH-3/4 dynamic shifts) */
static void SHAD(uint16_t opcode)
{
...
static void SHLD(uint16_t opcode)
{
    uint32_t m = REG_M, n = REG_N;
    if ((st->r[m] & 0x80000000) == 0)
        st->r[n] = st->r[n] << (st->r[m] & 0x1F);
...
static CV1K_ALWAYS_INLINE bool check_pending_irq(void)
{
...
static CV1K_ALWAYS_INLINE int sh34_step(void)
{
...
void sh7709s_reset(struct sh7709s_cpu *cpu)
{
    ...
    /* Default idle-loop PCs (ddpdfk/ddpsdoj); romset loader overrides per game. */
    cpu->idle_pc0 = 0x0c1d1346UL;
    cpu->idle_pc1 = 0x0c1d1348UL;
}
...
{
    cv1k_sh34_ctx = (struct sh34_cpu){ cpu, bus, 0 };
    return sh34_step();
}

CV1K_HOT cv1k_u32 sh7709s_run_linear_ops(struct sh7709s_cpu *cpu, struct cv1k_bus *bus,
                                          const cv1k_u16 *ops, cv1k_u32 op_count)
{
    if (cpu == NULL || bus == NULL || ops == NULL || op_count == 0U) return 0U;
    cv1k_sh34_ctx = (struct sh34_cpu){ cpu, bus, 0 };
    cv1k_u32 start = cpu->cycles;
    for (cv1k_u32 i = 0; i < op_count; i++) {
        if (cpu->sleep_mode == 1U || cpu->m_delay != 0U || cpu->halted != 0U) break;
        cpu->ppc = cpu->pc;
        cv1k_u16 opcode = ops[i];
        cv1k_sh34_ctx.cur_opcode = opcode;
        cpu->pc += 2U;
        cpu->icount = 0;
        execute_one(opcode);
        int consumed = 1 - (int)cpu->icount;
        if (consumed <= 0) consumed = 1;
        cpu->cycles += (cv1k_u32)consumed;
        if (cpu->m_delay != 0U) break;
        if (check_pending_irq()) break;
    }
...
CV1K_HOT cv1k_u32 sh7709s_run_until_idle(struct sh7709s_cpu *cpu, struct cv1k_bus *bus,
...
CV1K_HOT cv1k_u32 sh7709s_run_frame(struct sh7709s_cpu *cpu, struct cv1k_bus *bus,
...
CV1K_HOT cv1k_u32 sh7709s_run_frame_interpreter(struct sh7709s_cpu *cpu, struct cv1k_bus *bus,
                                      cv1k_u32 cycle_budget, cv1k_u32 tmu_interval)
{
    cv1k_sh34_ctx = (struct sh34_cpu){ cpu, bus, 0 };
