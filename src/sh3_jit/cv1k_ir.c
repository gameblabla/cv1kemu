...
/* Diagnostic toggle: when 0, memory ops always use the bus-helper call (no
 * inline fast-RAM path) — used by the microbench to measure the inline win. */
...
{
    size_t to_slow = 0, to_done = 0; int fast = g_ir_fastram_on;
    if (fast) {
...
/* SR.T = (the flags just set satisfy `setcc`).  Picks two temps from RAX/RCX/RDX
 * excluding the live operand regs x1/x2 (pass -1 if none).  Must be emitted
 * immediately after the flag-setting instruction.  Returns 0 if it cannot find
 * two free temps (caller then fails the block -> interpreter fallback). */
static int emit_t_from_flags(struct e64 *e, uint8_t setcc, int x1, int x2)
{
    int t1 = -1, t2 = -1; const unsigned cand[3] = { RAX, RCX, RDX };
    for (int i = 0; i < 3; i++) { int c = (int)cand[i]; if (c == x1 || c == x2) continue; if (t1 < 0) t1 = c; else if (t2 < 0) t2 = c; }
    if (t2 < 0) return 0;
    setcc_reg(e, setcc, (unsigned)t2);                 /* t2b = condition (0/1) */
    mov_r32_m32r12(e, (unsigned)t1, OFF_SR);
    alu_ri(e, 4, (unsigned)t1, ~1U);                   /* clear SR.T            */
    movzx_r32_r8(e, (unsigned)t2, (unsigned)t2);
    alu_rr(e, 0x09, (unsigned)t1, (unsigned)t2);        /* SR.T |= condition     */
...
/* Does this lowered buffer modify SR.T?  A delayed branch's delay slot must not
 * change T, since we execute it inline and then read T for the branch decision. */
static int iropbuf_writes_t(const struct iropbuf *b)
{
    for (int i = 0; i < b->n; i++) {
        cv1k_u8 o = b->in[i].op;
        if (o == IR_TSTORE || o == IR_DECT || o == IR_CMP ||
            o == IR_ROL || o == IR_ROR || o == IR_ROCL || o == IR_ROCR) return 1;
        if ((o == IR_SHL || o == IR_SHR || o == IR_SAR) && (b->in[i].flags & LFLAG_SETT)) return 1;
    }
    return 0;
}
                           ...
                           int *order, int *norder, int *has_mem)
{
    for (int i = 0; i < b->n; i++) {
        const struct cv1k_ir_inst *in = &b->in[i];
        if (in->op == IR_LOAD || in->op == IR_STORE) *has_mem = 1;
...
{
    *out_fn = NULL; *out_mem = NULL; *out_cap = 0;
    ...
    int has_mem = 0;
    int term = 0;                             /* 0 none, 1 BT/BF, 2 BT/S /BF/S   */
    ...
    for (; count < CV1K_IR_MAX_INSTS; count++) {
        ...
        if (hi == 0x8900U || hi == 0x8b00U || hi == 0x8d00U || hi == 0x8f00U) {
            ...
            if (delayed) {
                ...
                struct iropbuf db;
                if (!ir_lower(ds, term_bpc + 2U, &db) || db.nv > NSCRATCH || iropbuf_writes_t(&db)) break; /* can't lower delay slot -> branch is a fallback */
                ir_accum_masks(&db, &readmask, &writemask, order, &norder, &has_mem);
                term_delay = ds; term = 2; cycles += 2U;
            } else { term = 1; cycles += 1U; }
            break;
        }
        struct iropbuf b;
    ...
    /* delayed branch: execute the delay-slot op (inline) before the branch.  Its
     * IR was checked not to modify T, so reading T below matches hardware. */
    if (term == 2 && !e.fail) {
    ...
    if (term) {
        /* terminator: cpu->pc = (cond) ? taken : fall, computed here.  Delayed
         * forms already ran the delay slot, so fall = bpc+4 (else bpc+2). */
        cv1k_u32 taken = term_bpc + 4U + (cv1k_u32)((cv1k_s32)(int8_t)(term_op & 0xffU) * 2);
        cv1k_u32 fall = term_bpc + ((term == 2) ? 4U : 2U);
        unsigned hib = term_op & 0xff00U;
        int is_t = (hib == 0x8900U || hib == 0x8d00U);   /* BT/BT.S take if T set */
    ...
    *out_fn = (cv1k_ir_block_fn)mem; *out_mem = mem; *out_cap = cap;
    return count + (term == 1 ? 1U : term == 2 ? 2U : 0U);  /* guest insns covered */
#endif
}
...
struct ir_cached { cv1k_u32 pc; cv1k_ir_block_fn fn; void *mem; size_t cap; struct ir_cached *next; };
...
static cv1k_ir_block_fn irc_get(struct cv1k_bus *bus, cv1k_u32 pc)
{
    cv1k_u32 h = irc_hash(pc);
    for (struct ir_cached *c = g_irc[h]; c; c = c->next) if (c->pc == pc) return c->fn;
    ...
    if (c == NULL) { if (mem) cv1k_ir_free_block(mem, cap); return fn; }
    c->pc = pc; c->fn = fn; c->mem = mem; c->cap = cap; c->next = g_irc[h]; g_irc[h] = c;
    return fn;
}
                              ...
                              cv1k_u32 cycle_budget, cv1k_u32 tmu_interval)
{
    ...
    while ((cv1k_s32)(cpu->cycles - frame_end) < 0) {
        ...
        if (cpu->sleep_mode == 0U && cpu->m_delay == 0U && cpu->halted == 0U) {
            cv1k_ir_block_fn fn = irc_get(bus, cpu->pc);
            if (fn != NULL) { fn(cpu, bus); used = 1; g_ir_hits++; if (irc_irq_can_accept(cpu)) sh7709s_accept_pending_irq(cpu, bus); }
        }
