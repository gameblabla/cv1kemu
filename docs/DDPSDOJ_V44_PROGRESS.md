# DDPSDOJ v44 MAME-matching progress

v44 adds the missing cancellation side of the SH7709S TMU interrupt path.  Earlier builds modeled TMU underflow and could request a diagnostic TUNI event, but did not model the MAME behavior where writing `TCR0`, `TCR1`, or `TCR2` clears the pending TUNI interrupt when either `TIE` or `UNF` is clear.

MAME source basis: `src/devices/cpu/sh/sh4tmu.cpp`, where the `tcr*_w` handlers call `sh4_exception_unrequest(SH4_INTC_TUNI*)` if `!(TCR & 0x20) || !(TCR & 0x100)`.
...
Result: still not title-screen capable.  This patch prevents stale optional TMU events from staying latched after game code acknowledges/clears TCR state, but the larger SH7709S platform gaps remain: full scheduler/interrupt timing, MMU/TLB/cache behavior, DMAC/cache coherency, and remaining CV1000 FPGA/blitter/audio fidelity.
