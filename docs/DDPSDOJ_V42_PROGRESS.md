# DDPSDOJ v42 MAME-matching progress

v42 matches another SH7709S reset-context detail from MAME:

- Reset PC is now `0xa0000000` (P2 reset area), matching `sh34_base_device::device_reset()`.
- After U4 is loaded, the initial stack pointer is mirrored from boot ROM offset `0x04`, matching MAME's reset `read_long(4)` path.
- Save-state magic is now `CV1KSS42`.

The address translator maps P2 to physical boot ROM, so this is primarily a framework-fidelity and trace-correctness patch rather than a visible boot breakthrough.
...
Result: still not title-screen capable. The default run remains in the later copied-RAM allocator/list loop near `0c30b6ca`; the MAME-speedup diagnostic remains in the DDPSDOJ MAME idle-PC loop at `0c1d1346`.
